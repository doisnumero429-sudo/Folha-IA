...
export default function Step2Page() {
  ...
  async function handleUpload(file) {
    ...
    try {
      ...
      const res = await api.post('/upload/consumo', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      setResult(res.data)
    } catch (err) {
      setUploadError(err.response?.data?.error || 'Erro ao processar arquivo')
  ...
  async function resolveAmbiguous(alias, funcionarioId) {
    ...
    try {
      ...
      setResult(prev => {
        ...
        const resolved = (prev.ambiguous || prev.ambiguos || []).find(a => a.alias === alias)
        const newMatched = resolved
          ? [...matched, { ...resolved, funcionario_id: funcionarioId, resolved: true }]
          : matched
        return { ...prev, ambiguous: newAmbiguous, ambiguos: newAmbiguous, matched: newMatched }
      })
    } catch (err) {
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-stone-400 text-sm mt-1">
            Faça upload do relatório de consumo (.xls) exportado do sistema.
          </p>
        </div>
            ...
            {blocked.length > 0 && (
              ...
              >
                <p className="text-red-300 text-xs mb-3">
                  Nomes que não puderam ser processados. Verifique no painel de correlações.
                </p>
            ...
            {notFound.length > 0 && (
              ...
              >
                <p className="text-orange-300 text-xs mb-3">
                  Linhas com token FUNCIONARIO que não corresponderam a nenhum funcionário.
                </p>
                <div className="space-y-1.5">
                  {notFound.map((n, idx) => (
                    <div key={idx} className="flex items-center gap-2 text-sm">
