  ...
  async function handleUpload(file) {
    ...
    try {
      const [uploadRes, empRes] = await Promise.all([
        api.post('/upload/vales', (() => {
          const f = new FormData()
          f.append('file', file)
          f.append('fechamento_id', id)
          return f
    ...
    <Layout>
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-stone-400 text-sm mt-1">Faça upload do relatório de vales (.html) exportado do sistema.</p>
        </div>
            ...
            {notFound.length > 0 && (
              <Section title="Não Encontrados — Criar Correlação" color="bg-orange-900/10" borderColor="#f97316" count={notFound.length}>
                <p className="text-orange-300 text-xs mb-3">
                  Esses nomes têm token FUNCIONARIO mas não foram identificados. Crie uma correlação para vinculá-los.
                </p>
        ...
        {result && !result.error && (
          <div className="flex justify-between mt-8">
            <button onClick={() => navigate(`/fechamento/${id}/consumo`)} className="px-5 py-2.5 rounded-lg border font-medium text-stone-300 hover:bg-stone-800 transition-colors" style={{ borderColor: '#3c3330' }}>
              ← Voltar
            </button>
        ...
        {!result && (
          <div className="flex justify-between mt-8">
            ...
            <button onClick={() => navigate(`/fechamento/${id}/faltas`)} className="px-6 py-2.5 rounded-lg border font-medium transition-colors" style={{ borderColor: '#3c3330', color: '#78716c' }}>
              Pular →
            </button>
          </div>
        )}
